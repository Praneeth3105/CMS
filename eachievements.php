...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // achievement_link is now a plain URL/link entered by the user, not an uploaded file.
    $achievement_link = trim($_POST['achievement_link'] ?? '');
    if ($achievement_link === '') {
        $achievement_link = $row['achievement_link'];
    }

    if (empty($errorMsg)) {
        $sql = "UPDATE `achievements` SET `academic_year` = ?, `faculty_name` = ?, `award_name` = ?, `description` = ?, `achievement_date` = ?, `organization` = ?, `achievement_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "ssssssss", $academic_year, $faculty_name, $award_name, $description, $achievement_date, $organization, $achievement_link, $id);
...
<body>
    <div class="wrap">
        ...
        <div class="subtitle">Table: achievements &middot; Record ID: <?php echo htmlspecialchars($id); ?></div>
        ...
        <div class="card">
            ...
            <form method="POST">
                ...
                <div class="field">
                    ...
                    <input type="text" name="award_name" id="award_name" value="<?php echo htmlspecialchars($row['award_name'] ?? ''); ?>">
                </div>
                ...
                <div class="field">
                    ...
                    <input type="text" name="achievement_date" id="achievement_date" value="<?php echo htmlspecialchars($row['achievement_date'] ?? ''); ?>">
                </div>
                ...
                <div class="field full">
                    <label for="achievement_link">Proof / Certificate Link (paste a URL; leave empty to keep the current link)</label>
                    <input type="url" name="achievement_link" id="achievement_link" placeholder="https://...">
                    <?php if (!empty($row['achievement_link'])): ?>
                        <div class="current-file">Current link: <a href="<?php echo htmlspecialchars($row['achievement_link']); ?>" target="_blank" rel="noopener"><?php echo htmlspecialchars($row['achievement_link']); ?></a></div>
                    <?php else: ?>
