...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm'])) {
    $stmt = mysqli_prepare($conn, "DELETE FROM extracircular WHERE rollno=? AND eventname=?");
    mysqli_stmt_bind_param($stmt, "ss", $rollno, $name);
    if (mysqli_stmt_execute($stmt)) {
        if (!empty($file) && file_exists(__DIR__ . '/images/' . $file)) {
            @unlink(__DIR__ . '/images/' . $file);
        }
        header("Location: ssearch.php?deleted=extracircular");
        exit;
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Delete Extra Circular</title>
