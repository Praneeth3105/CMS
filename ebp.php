...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `bookpublish` SET `academic_year` = ?, `month` = ?, `faculty_name` = ?, `no_of_authors` = ?, `author_position` = ?, `title` = ?, `publisher` = ?, `scopus_sci` = ?, `url` = ?, `isbn` = ?, `doi` = ?, `proof_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssssssssss", $academic_year, $month, $faculty_name, $no_of_authors, $author_position, $title, $publisher, $scopus_sci, $url, $isbn, $doi, $proof_link, $id);
