...
{
    $stmt = mysqli_prepare($conn, "SELECT * FROM `bookpublish` WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $scopus_sci = trim($_POST['scopus_sci'] ?? '');
    $url = trim($_POST['url'] ?? '');
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `bookpublish` SET `academic_year` = ?, `month` = ?, `faculty_name` = ?, `no_of_authors` = ?, `author_position` = ?, `title` = ?, `publisher` = ?, `scopus_sci` = ?, `url` = ?, `isbn` = ?, `doi` = ?, `start_date` = ?, `end_date` = ?, `proof_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssssssssssss", $academic_year, $month, $faculty_name, $no_of_authors, $author_position, $title, $publisher, $scopus_sci, $url, $isbn, $doi, $start_date, $end_date, $proof_link, $id);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Book Published</title>
...
<body>
    <div class="wrap">
        ...
        <h1>Edit Book Published</h1>
        <div class="subtitle">Table: bookpublish &middot; Record ID: <?php echo htmlspecialchars($id); ?></div>
        ...
        <div class="card">
            ...
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="field">
                    ...
                    <input type="text" name="author_position" id="author_position" value="<?php echo htmlspecialchars($row['author_position'] ?? ''); ?>">
                </div>
                ...
                <div class="field">
                    ...
                    <input type="text" name="publisher" id="publisher" value="<?php echo htmlspecialchars($row['publisher'] ?? ''); ?>">
                </div>

                <div class="field">
                    <label for="scopus_sci">Scopus / SCI</label>
                    <input type="text" name="scopus_sci" id="scopus_sci" value="<?php echo htmlspecialchars($row['scopus_sci'] ?? ''); ?>">
                </div>
                ...
                <div class="field">
                    ...
                    <input type="text" name="isbn" id="isbn" value="<?php echo htmlspecialchars($row['isbn'] ?? ''); ?>">
                </div>
