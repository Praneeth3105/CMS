...
{
    $stmt = mysqli_prepare($conn, "SELECT * FROM `reviewer_activities` WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `reviewer_activities` SET `academic_year` = ?, `month` = ?, `faculty_name` = ?, `date_attended` = ?, `organization` = ?, `conference_journal_name` = ?, `type` = ?, `start_date` = ?, `end_date` = ?, `proof_link` = ? WHERE id = ?";
...
<head>
    <meta charset="UTF-8">
    <title>Edit Reviewer Activity</title>
...
<body>
    <div class="wrap">
        ...
        <h1>Edit Reviewer Activity</h1>
        <div class="subtitle">Table: reviewer_activities &middot; Record ID: <?php echo htmlspecialchars($id); ?></div>
