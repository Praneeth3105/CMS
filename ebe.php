...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `bookedited` SET `faculty_name` = ?, `no_of_authors` = ?, `book_name` = ?, `publisher_name` = ?, `isbn_number` = ?, `url` = ?, `academic_year` = ?, `month` = ?, `proof_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "ssssssssss", $faculty_name, $no_of_authors, $book_name, $publisher_name, $isbn_number, $url, $academic_year, $month, $proof_link, $id);
