...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `working_models` SET `academic_year` = ?, `model_name` = ?, `duration` = ?, `students_count` = ?, `domain_name` = ?, `proof_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssss", $academic_year, $model_name, $duration, $students_count, $domain_name, $proof_link, $id);
