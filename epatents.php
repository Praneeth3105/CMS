...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `patents` SET `academic_year` = ?, `month` = ?, `faculty_name` = ?, `patent_details` = ?, `area_of_patent` = ?, `application_number` = ?, `status` = ?, `patent_type` = ?, `filing_agency` = ?, `proof_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssssssss", $academic_year, $month, $faculty_name, $patent_details, $area_of_patent, $application_number, $status, $patent_type, $filing_agency, $proof_link, $id);
