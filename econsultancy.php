...
{
    $stmt = mysqli_prepare($conn, "SELECT * FROM `consultancy_work` WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $students_involved = trim($_POST['students_involved'] ?? '');
    ...
    if (empty($errorMsg)) {
        $sql = "UPDATE `consultancy_work` SET `academic_year` = ?, `faculty_name` = ?, `description` = ?, `organization` = ?, `amount` = ?, `start_date` = ?, `end_date` = ?, `duration` = ?, `students_involved` = ?, `proof_link` = ? WHERE id = ?";
        $stmt = mysqli_prepare($conn, $sql);
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "sssssssssss", $academic_year, $faculty_name, $description, $organization, $amount, $start_date, $end_date, $duration, $students_involved, $proof_link, $id);
...
<body>
    <div class="wrap">
        ...
        <h1>Edit Consultancy Work</h1>
        <div class="subtitle">Table: consultancy_work &middot; Record ID: <?php echo htmlspecialchars($id); ?></div>
        ...
        <div class="card">
            ...
            <form method="POST">
                ...
                <div class="field">
                    <label for="students_involved">Students Involved</label>
                    <input type="text" name="students_involved" id="students_involved" value="<?php echo htmlspecialchars($row['students_involved'] ?? ''); ?>">
                </div>
