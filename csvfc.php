...
<head>
    <meta charset="UTF-8">
    <title>Faculty Certificates CSV Upload | Certificate Management System</title>
...
<body>
    ...
    <div class="page-hero">
        ...
        <h2>Faculty Certificates <span class="accent">CSV Upload</span></h2>
        <p>Upload a CSV file to bulk-import faculty certificate records.</p>
    </div>
    ...
    if (isset($_FILES['csvFile']) && $_FILES['csvFile']['error'] == 0) {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== false) {
            ...
            $start_date = parseFlexibleDate($start_date_raw);
            $end_date = parseFlexibleDate($end_date_raw);

            $rowHasBadDate = false;
            if ($start_date_raw !== '' && $start_date === null) {
                $unparsedDates[] = "Row $rowNum, Start Date: \"$start_date_raw\"";
                $rowHasBadDate = true;
            }
            if ($end_date_raw !== '' && $end_date === null) {
                $unparsedDates[] = "Row $rowNum, End Date: \"$end_date_raw\"";
                $rowHasBadDate = true;
            }
            ...
            foreach ($data as $colIndex => $value) {
                $cellClass = '';
                if ($rowHasBadDate && ($colIndex === 4 || $colIndex === 5)) {
